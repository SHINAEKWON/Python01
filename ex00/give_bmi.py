...
def give_bmi(height: list[int | float], weight: list[int | float]) -> list[int | float]:
    """
    This program takes as input a list of height in meter and a list of weight in kg.
...
def apply_limit(bmi: list[int | float], limit: int) -> list[bool]:
    ...
    If a BMI is equal or bigger than the second input, it return True, otherwise, it returns False.
    """
    bmi_tf = []
    try:
        if len(bmi) == 0:
